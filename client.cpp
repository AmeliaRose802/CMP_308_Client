/*	AG0907 Lab 5 TCP client example - by Henry Fortuna and Adam Sampson
...
int main()
{
	...
	buffer[MESSAGESIZE-1] = '\0'; //Need to make it null terminated 

	std::string line(buffer);

	if (!line.compare(0, 3, "429")) {
		printf(line.c_str() + '\n');
		die("Server politely closed connection");
	}
	printf(line.c_str());
	...
	while (true)
	{
		...
		if (!messageStr.compare(0, 3, "420")) {
			printf(messageStr.c_str() + '\n');
			die("Server closed connection due to idle client");
		}
